...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	chatService := service.NewChatService(chatRepository, apiKeyService, imageStorageSettingService)
	...
	imageAssetRepository := repository.NewImageAssetRepository(db)
	imageAssetCache := repository.NewImageAssetCache(redisClient)
	imageAssetService := service.NewImageAssetService(imageAssetRepository, imageAssetCache, imageStorageSettingService)
	imageTaskService := service.ProvideImageTaskService(imageTaskStore, imageStorageSettingService, imageAssetService)
	asyncImageHandler := handler.NewAsyncImageHandler(imageTaskService, imageAssetService, openAIGatewayHandler)
	...
	imageAssetCleanupService := service.ProvideImageAssetCleanupService(imageAssetService)
	batchImageHandler := handler.ProvideBatchImageHandler(batchImagePublicService, batchImageDownloadService, batchImageCleanupService, openAIGatewayHandler)
	chatHandler := handler.ProvideChatHandler(chatService, openAIGatewayHandler, gatewayHandler, subscriptionService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, chatHandler, idempotencyCoordinator, idempotencyCleanupService)
