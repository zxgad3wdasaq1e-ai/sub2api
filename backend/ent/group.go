...
type Group struct {
	...
	// Whether each user may purchase this subscription group only once
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldOneTimeSubscription:
			if value, ok := values[i].(*sql.NullBool); !ok {
